...
export class LiveDataProvider implements DataProvider {
  ...
  private hydrateInitialBaseline(): void {
    const lookup = getScenario("nominal");
    if (lookup.ok) {
      for (const step of lookup.scenario.steps) {
        if (step.emit) {
          const emitObj = step.emit as Record<string, unknown>;
          const nonVehicleEmit: Record<string, unknown> = {};
          for (const [k, v] of Object.entries(emitObj)) {
            if (k !== "VehicleState") {
              nonVehicleEmit[k] = v;
            }
          }
          if (Object.keys(nonVehicleEmit).length > 0) {
            this.ingest({ emit: nonVehicleEmit });
          }
        }
      }
    }
